<script lang="ts">
	...
	const COMMANDS: WireEntry[] = [
		...
		},
		{
			...
			request: ["7 (u8)", "<key length> (u32)", "<key> (u8[])"],
			response: ["<value length> (u32)", "<value> (u8[])"],
		},
		{
			label: "TTL",
		...
		},
		{
			...
			request: ["12 (u8)", "<policy length> (u32)", "<policy> (u8[])"],
			response: [],
		},
		{
			label: "STATS",
